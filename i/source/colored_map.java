...
public class colored_map extends IApplication {
	MainCanvas gc;
...
*/
	...
	public void paint(Graphics g){
		...
		g.lock();
			...
			for(i=0;i<10;i++){
				for(j=0;j<10;j++){
					g.setColor(color_tbl[ field[i][j] ] );
					g.fillRect(i*12,j*12,12,12);
					if( i!=9 && label_field[i][j] !=label_field[i+1][j]){//右隣が違うラベルだったら境界線を引く
						g.setColor(Graphics.getColorOfName(Graphics.BLACK));
						g.fillRect(i*12+10,j*12,2,12);
					}

					if( j!= 9 && label_field[i][j]!=label_field[i][j+1]){//下隣が違うラベルだったら境界線を引く
						g.setColor(Graphics.getColorOfName(Graphics.BLACK));
						g.fillRect(i*12-2,j*12+10,12+2,2);
					}
				}
			}
			...
			g.fillRect(120-2,0,2,120);
			g.fillRect(0,120-2,120,2);


			//カーソルを表示する
			if(flag==0){
				g.setColor(Graphics.getColorOfName(Graphics.BLACK));
				g.fillRect(cursor_x*12+2,cursor_y*12+2,4,4);
			}

			if(flag==1){//完成状態ならば祝福メッセージを表示
			g.setColor(Graphics.getColorOfName(Graphics.WHITE));
			g.drawString("完成!!",70,80);
			g.drawString("完成!!",71,80);
