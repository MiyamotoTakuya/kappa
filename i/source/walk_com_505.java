...
public class walk_com_505 extends IApplication {
	MainCanvas gc;
	...
	*/
	...
	public void paint(Graphics g){
		...
		if(mode == 0){//タイトル画面モード
			...
			g.drawString("　　いい加減",0,240/2-12);



			g.drawString("　　MAX COMBO : "+max_combo_count,0,239-24-24);
			...
			g.drawImage(img[13],cur_pos*24*2-24+4,0);
			g.drawImage(img[Math.abs(kou[0])],0*24+4,30);

			j=1;
			for(i=1;i<=4;i++){
				if(kou[i]>0){
					g.drawImage(img[10],j*24+4,30);//”＋”を表示
				}else {
					g.drawImage(img[11],j*24+4,30);//”－”を表示
				}
				j++;
				g.drawImage(img[Math.abs(kou[i])],j*24+4,30);
				j++;
			}

			j=1;
			g.drawImage(img[12],j*24+4,30+32+2);//”＝”の表示

			if(kou[5]<0){
				j++;
				g.drawImage(img[11],j*24+4,30+32+2);//”－”を表示
			}
			if( (kou[5]>=10)||(kou[5]<=-10) ){
				j++;
				g.drawImage(img[Math.abs(kou[5])/10],j*24+4,30+32+2);//１０の位表示

			}
			j++;
			g.drawImage(img[Math.abs(kou[5])%10],j*24+4,30+32+2);//１の位表示


			if(flag==1){//正解時
				g.drawString("成立じゃ。",24,120);
			}else if(flag==2){//不正解時
				g.setColor(Graphics.getColorOfName(Graphics.RED));
				g.drawString("不成立じゃ。",24,120);

			}
			g.drawString("COMBO : "+combo_count,24,120+24);
